...
def establish_concurrent_state(
  ...
  kindred.replace_editor_text(local_text)
  assert kindred.sync_state()["localDirty"] is True
  assert kindred.sync_state()["baseRevisionId"] == base_revision

  remote_revision = google_docs.set_text(remote_text)
...
def test_concurrent_non_overlapping_inserts_merge(kindred, google_docs):
  base, remote = establish_concurrent_state(
    kindred, google_docs, "AAA LOCAL BBB CCC", "AAA BBB REMOTE CCC"
  )
  breakpoint()
  state = kindred.sync_state()
  breakpoint()
...
@pytest.mark.parametrize(
  ("base", "local", "remote", "expected_markers"),
  [
    ("abcdef", "abLOCALcdef", "abcdREMOTEef", ("LOCAL", "REMOTE")),
    ("AAA BBB CCC", "AAA CCC", "AAA BBB REMOTE CCC", ("REMOTE",)),
    ("AAA BBB CCC", "AAA LOCAL BBB CCC", "AAA CCC", ("LOCAL",)),
    ("AAA BBB CCC", "AAA LOCAL BBB LOCAL2 CCC", "AAA BBB REMOTE CCC", ("LOCAL", "LOCAL2", "REMOTE")),
  ],
)
def test_concurrent_edit_matrix(kindred, google_docs, base, local, remote, expected_markers):
  ...
  wait_for_sync_idle(kindred)
  kindred.pause_sync()
  kindred.replace_editor_text(local)
  remote_revision = google_docs.set_text(remote)
  assert remote_revision != base_revision
  kindred.resume_sync()
  WebDriverWait(kindred.driver, 30).until(lambda _: not kindred.sync_state().get("localDirty"))
  wait_for_sync_idle(kindred)
  final_text = kindred.editor_text()
  remote_text = google_docs.text()
  for marker in expected_markers:
    assert marker in final_text
    assert marker in remote_text
  assert final_text == remote_text
